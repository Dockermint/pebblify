...
| 6 | Telegram notifier library         | Raw `net/http` + `encoding/json` (stdlib). No third-party lib. `go-telegram-bot-api/v5` rejected: abandoned (last release 2021-12-13, last commit 2022-10-20, 110+ open issues). |
| 7 | API always-on in daemon mode      | `[api].enable` field removed. API always active in daemon mode — no opt-out flag. `host`, `port`, `authentification_mode` remain configurable. *(CEO amendment 2026-04-18)* |
...
| `net/http` + `encoding/json` (stdlib)                | Telegram notify          | None       | **CEO locked** — no third-party lib. `go-telegram-bot-api/v5` rejected (abandoned). Only `POST /sendMessage` needed. |
| `golang.org/x/crypto`                          | SCP/SSH transport               | Low        | Already indirect; promote to direct                                        |

Telegram notifier uses `net/http` + `encoding/json` (stdlib) exclusively. `go-telegram-bot-api/v5` is not used. Zero additional deps for notification.
...
Step 6:  @lead-dev  — add TOML, aws-sdk-go-v2 (3 sub-modules only), x/crypto; no Telegram lib (stdlib only)
...
- **Telegram notifications**: opt-in notify on job completion or failure via raw
  `net/http` POST to the Telegram Bot API. No third-party library; stdlib only.
...
- **TOML config file**: `config.toml` with versioned schema (config_version = 0)
  covers API (always-on; no enable flag), notify, telemetry, health, conversion,
  and save targets.
